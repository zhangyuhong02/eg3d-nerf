...
def train(audio_path, img_path, model_path, save_path, audio_feature_path=None, epoch=50, mode="train"):
    ...
    rendering_kwargs = {}
    rendering_kwargs["neural_rendering_resolution"] = 64
        ...
        for i in tqdm(range(epoch)):
            ...
            for bs_idx in range(bs):
                ...
                for frame_bs_idx in range(startid, end_id):
                    ...
                    #print("feature map shape:", feature_map.shape) # ([1, 64, 256, 256])
                    ...
                    #print("feature_samples.shape",feature_samples.shape) # torch.Size([1, 4096, 32])
                    ...
                    rgb_image = feature_image[:, :3] # [1, 3, 64, 64]
                    out = F.interpolate(input=rgb_image, scale_factor=4.0, mode="bilinear", align_corners=False)
        ...
        for bs_idx in range(bs):
            ...
            for frame_bs_idx in range(startid, end_id):
                ...
                predictions_gen.append(
                    (np.transpose(out.data.cpu().numpy(), [0, 2, 3, 1])[0] * 255).astype(np.uint8))       


                total_frame += 1
                if total_frame >= frames:
                    break            
            if total_frame >= frames:
                break
